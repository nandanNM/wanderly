        ...
        {media.length === 0 ? (
          ...
          <>
                  ...
                  <div className="absolute right-1.5 top-1.5 flex gap-1 opacity-0 transition-opacity group-hover:opacity-100">
                    {allowDownloads && (
                      <button
                        ...
                        aria-label="Download"
                        title="Download"
                        className="grid h-7 w-7 place-items-center rounded-full border border-black/10 bg-white/90 text-[#5a5a5a] shadow-sm hover:text-[#5a7d2e] disabled:opacity-50"
                      >
                        {downloadingId === m.id ? "…" : "⬇"}
                      </button>
                    )}
                    {m.canDelete && (
                      <button
                        ...
                        aria-label="Delete"
                        title="Delete"
                        className="grid h-7 w-7 place-items-center rounded-full border border-black/10 bg-white/90 text-[#5a5a5a] shadow-sm hover:text-red-600 disabled:opacity-50"
                      >
                        {deletingId === m.id ? "…" : "✕"}
                      </button>
                    )}
            ...
            {visibleCount < media.length && (
              <div
                ref={sentinelRef}
                className="mt-4 flex justify-center py-4 text-sm text-[#9a9a9a]"
              >
                Loading more…
              </div>
            )}
